...
export default class NeonGridGameScene extends Phaser.Scene {
  constructor() {
    super({ key: 'NeonGridGameScene' });
  ...
  create() {
    console.log('NeonGridGameScene: Create');
    ...
    if (!this.gameConfig || !this.slotId) {
        console.error("NeonGridGameScene: Missing game configuration or Slot ID!");
        console.error("slotApiData:", slotApiData);
        console.error("slotGameJsonConfig:", slotGameJsonConfig);
        EventBus.$emit('phaserError', 'NeonGridGameScene: Game initialization failed: Missing configuration.');
        return;
    }
    ...
    console.log('NeonGridGameScene: Ready.');
  ...
  shutdown() {
    console.log('NeonGridGameScene: shutdown');
 ...
 createReels() {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      for (let i = 0; i < symbolsToCreatePerReel; i++) {
        ...
        const symbolKey = `${this.slotId}_symbol_${randomSymbolConfig.id}`; // Use the correct key format from PreloadScene
  ...
  createBorders() {
    ...
    const outerColor = 0x00ffff; // Neon Cyan
    const innerColor = 0xff00ff; // Neon Magenta
    const alpha = 0.8;
 ...
 // --- Spin Logic ---
  ...
  handleSpinResult(responseData) {
      console.log('NeonGridGameScene received spin result from Vuex:', responseData);
  ...
  spinReel(reelIndex, reelSymbols, baseDuration, targetSymbolIdsForThisReel, onCompleteCallback) { // D
    ...
    reelSymbols.forEach((symbol, i) => {
        if (i >= this.gridConfig.maxRowsForSymbolMap) {
            ...
            const randomSymbolConfig = this.getRandomSymbolConfig();
            symbol.setTexture(`${this.slotId}_symbol_${randomSymbolConfig.id}`);
        ...
        onComplete: () => { // D.1
            ...
            reelSymbols.forEach((symbol, i) => { // D.1 Loop through all symbols in reel column
                if (i < landedPanesCount) {
                    ...
                    if (symbolConfig) {
                         symbol.setTexture(`${this.slotId}_symbol_${symbolConfig.id}`);
                    }
  ...
  onAllReelsStopped() {
      console.log('NeonGridGameScene: All reels stopped.');
      this.isSpinning = false;
      ...
      if (bonusConfig && bonusConfig.triggerSymbolId && bonusConfig.minTriggerCount && this.lastSpinResponse?.result) {
          ...
          if (triggerCount >= bonusConfig.minTriggerCount) {
              ...
              this.scene.pause('NeonGridUIScene');
...
clearWinAnimations(clearTotalWinText = true) { // H
  ...
  playCascadeSettleEffect() {
      ...
      console.log('NeonGridGameScene: Playing cascade settle effect');
  ...
  handleBonusComplete(data) {
      console.log('NeonGridGameScene: Bonus game complete', data);
      this.scene.resume('NeonGridGameScene');
      this.scene.resume('NeonGridUIScene');
