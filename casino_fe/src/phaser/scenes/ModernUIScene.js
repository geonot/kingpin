...
export default class ModernUIScene extends Phaser.Scene {
  constructor() {
    super({ key: 'ModernUIScene' });
  ...
  create() {
    console.log('ModernUIScene: Create');
    const gameConfig = this.registry.get('gameConfig');
    if (!gameConfig) {
        console.error("ModernUIScene: Game config not found!");
        return;
    }
    ...
    EventBus.$on('uiSetIdle', () => {
     ...
     // Listen for settings changes to update button states if needed (e.g., Turbo)
      ...
      console.log('ModernUIScene: Ready.');
  }
   ...
   createBetDisplay(initialBet, sizeConfig, adjustConfig) {
    ...
    createActionButtons(config) {
        ...
        this.spinButton.on('pointerdown', () => {
            const gameScene = this.scene.get('ModernSlotScene');
            ...
            // EventBus.$emit('getBalanceForDeduction', this.currentBetSats); // Removed as per review
        ...
        const settingsConfig = config?.settings;
         if (settingsConfig) {
            ...
            this.settingsButton.on('pointerdown', () => {
                 ...
                 this.scene.pause('ModernUIScene'); // Pause this scene as well
