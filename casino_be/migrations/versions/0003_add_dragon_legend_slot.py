...
revision = '0003'
down_revision = '0002' # Depends on Hack the Planet slot
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # Insert the Dragon Legend slot entry
    op.bulk_insert(slot_table,
        [
            {
                'id': 2, # Ensure ID is unique
                'name': 'Dragon Legend',
                'description': 'Face the mighty dragon in this legendary slot game.',
                'num_rows': 3,
                'num_columns': 5,
                'num_symbols': 14, # Matches gameConfig
                'wild_symbol_id': 14, # Matches gameConfig (symbol_wild)
                ...
                'short_name': 'dragon', # For path /slot2/
                'asset_directory': 'slot2', # Matches path
                'is_active': True,
                'created_at': now_utc
            }
        ]
    ...
    # Insert the Dragon Legend slot symbols
    ...
    # Insert the Dragon Legend slot bet options
    op.bulk_insert(slot_bet_table,
        [
            {'slot_id': 2, 'bet_amount': 10 * 100000000, 'created_at': now_utc},  # 10 BTC in satoshis
            {'slot_id': 2, 'bet_amount': 20 * 100000000, 'created_at': now_utc},  # 20 BTC in satoshis
            {'slot_id': 2, 'bet_amount': 50 * 100000000, 'created_at': now_utc},  # 50 BTC in satoshis
            {'slot_id': 2, 'bet_amount': 100 * 100000000, 'created_at': now_utc}, # 100 BTC in satoshis
            {'slot_id': 2, 'bet_amount': 200 * 100000000, 'created_at': now_utc}, # 200 BTC in satoshis
            {'slot_id': 2, 'bet_amount': 500 * 100000000, 'created_at': now_utc}  # 500 BTC in satoshis
        ]
    )

def downgrade():
    # Delete the inserted Dragon Legend slot entry and related data
    op.execute(
        slot_bet_table.delete().where(slot_bet_table.c.slot_id == 2)
    )
    op.execute(
        slot_symbol_table.delete().where(slot_symbol_table.c.slot_id == 2)
    )
    op.execute(
        slot_table.delete().where(slot_table.c.id == 2)
    )
