...
revision = '0005'
down_revision = '0004' # Depends on Synthwave slot being added
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # Insert the Fruit and Cream slot entry
    op.bulk_insert(slot_table,
        [
            {
                'id': 4, # Ensure ID is unique
                'name': 'Fruit and Cream',
                'description': 'Immerse yourself in the yummy fruit and cream desert spectacular.',
                'num_rows': 3,
                'num_columns': 5,
                'num_symbols': 12, # Matches gameConfig
                'wild_symbol_id': 12, # Matches gameConfig (symbol_wild)
                'scatter_symbol_id': 11, # Matches gameConfig (symbol_scatter)
                'bonus_type': 'free_spins',
                'bonus_subtype': 'standard',
                ...
                'short_name': 'fruit', # For path /slot4/
                'asset_directory': 'slot4', # Matches path
                'is_active': True,
                'created_at': now_utc
            }
        ]
    ...
    # Insert the Fruit and Cream slot symbols
    ...
    # Insert the Fruit and Cream slot bet options
    op.bulk_insert(slot_bet_table,
        [
            {'slot_id': 4, 'bet_amount': 10 * 100000000, 'created_at': now_utc},  # 10 BTC in satoshis
            {'slot_id': 4, 'bet_amount': 20 * 100000000, 'created_at': now_utc},  # 20 BTC in satoshis
            {'slot_id': 4, 'bet_amount': 50 * 100000000, 'created_at': now_utc},  # 50 BTC in satoshis
            {'slot_id': 4, 'bet_amount': 100 * 100000000, 'created_at': now_utc}, # 100 BTC in satoshis
            {'slot_id': 4, 'bet_amount': 200 * 100000000, 'created_at': now_utc}, # 200 BTC in satoshis
            {'slot_id': 4, 'bet_amount': 500 * 100000000, 'created_at': now_utc}  # 500 BTC in satoshis
        ]
    )

def downgrade():
    # Delete the inserted Fruit and Cream slot entry and related data
    op.execute(
        slot_bet_table.delete().where(slot_bet_table.c.slot_id == 4)
    )
    op.execute(
        slot_symbol_table.delete().where(slot_symbol_table.c.slot_id == 4)
    )
    op.execute(
        slot_table.delete().where(slot_table.c.id == 4)
    )
